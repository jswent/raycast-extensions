  ...
  return (
    <List.Item
      ...
      actions={
        <ActionPanel>
          <ActionPanel.Section>
            {preferences.useNeovimInsteadOfOpenWith1 ? (
              <Action
                title="Open in Neovim (WezTerm)"
                icon={Icon.Terminal}
                onAction={() => {
                  openInNeovimWezTerm(repo.fullPath);
                  props.recordUsageHook?.(repo.name);
                }}
              />
            ) : (
            ...
            <Action
              ...
              onAction={() => {
                ...
                if (preferences.useNeovimInsteadOfOpenWith1) {
                  openInNeovimWezTerm(repo.fullPath);
                  props.recordUsageHook?.(repo.name);
                } else {
...
function openInNeovimWezTerm(repoPath: string) {
  ...
  exec(`zsh -il -c "wezterm cli spawn --new-window --cwd ${escapedPath} nvim ${cmdArgs} ."`, (error) => {
    if (error) {
      showToast({
        ...
        title: "Failed to open Neovim in WezTerm",
        message: error.message,
      });
    } else {
      // Close Raycast window when successfully opening Neovim in WezTerm
      closeMainWindow();
    }
  });
}
